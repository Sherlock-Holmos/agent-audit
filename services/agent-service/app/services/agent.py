...
─────────────────────────────────────────────────────────────────────────────
当前链路：
  System Prompt (含 dashboard 数据) + 对话历史 → LLM → StrOutputParser

接入 RAG 时只需：
  1. 在 docker-compose 启动向量数据库（Chroma / pgvector）
  2. 设置环境变量  VECTOR_STORE_TYPE=chroma  （或 pgvector）
  3. 首次摄入知识文档（见 scripts/ingest.py，待实现）
  → _build_retriever() 会自动返回 Retriever，注入 rag_context

LLM 提供商（LLM_PROVIDER）：
  mock   — 无需 API Key，开发调试用
  openai — OPENAI_API_KEY + OPENAI_MODEL
  azure  — AZURE_OPENAI_* 系列变量
─────────────────────────────────────────────────────────────────────────────
"""
...
async def run_agent(
    ...
    Args:
        question: 本轮用户问题
        history:  Redis 中的历史记录列表，每条含 "q" / "a" 字段
